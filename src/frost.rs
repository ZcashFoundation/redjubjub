...
/// Commitment is a Jubjub point that is a (public) commitment to one
/// coefficient of a secret polynomial used for performing Verifiable secret
/// sharing for a Shamir secret share.
#[derive(Clone)]
...
/// generate each ith participants key share f(i). Participants use this set of
/// commitments to perform verifiable secret sharing. Note that participants
/// MUST be assured that they have the *same* ShareCommitment, either by
/// performing pairwise comparison or by using some agreed-upon public location
/// for publication where each participant can ensure that they received
/// the correct (and same) value.
#[derive(Clone)]
...
pub struct SharePackage {
    ...
    pub index: u32,
    /// Share is this participant's share
    pub(crate) share: Share,
    /// Public is this participant's public key
    pub(crate) public: Public,
    /// GroupPublic is the public signing key that represents the entire group.
...
impl TryFrom<SharePackage> for KeyPackage {
    ...
    /// when participants receive a SharePackage from the dealer, they *MUST*
    /// verify the integrity of the share before continuing on to transform it
    /// into a signing/verification keypair. Here, we assume that every
    /// participant has the same view of the commitment issued by the dealer,
    /// but implementations *MUST* make sure that all participants have a
    /// consistent view of this commitment in practice.
...
/// KeyPackage is a FROST keypair. Here, this keypair is generated by a trusted
/// dealer; in the future we will add support for using a DKG.
/// When using a central dealer, KeyPackages are derived from a SharePackage
/// that is generated by the dealer and then distributed to participants,
/// who first perform verification before deriving the KeyPackage. Participants
/// then store their KeyPackage for later use during signing.
...
/// central, trusted dealer. This action under the hood performs verifiable
/// secret sharing, which itself uses Shamir secret sharing, from which each
/// share becomes a participant's secret key.
...
/// ensuring that this participant's share has been generated using the same
/// mechanism as all other signing participants. Note that participants *must*
/// ensure that they have the same view as all other participants of the commitment!
...
/// SigningNonces are comprised of hiding and binding nonces. Note that
/// SigningNonces must be used *only once* for a signing operation; re-using
/// nonces will result in leakage of a signer's long-lived signing key.
#[derive(Clone)]
...
/// implementation. Each SigningCommitment can be used for exactly *one*
/// signature.
...
/// SigningPackage is generated by the coordinator of the signing operation
/// and distributed to each signing party.
pub struct SigningPackage {
    /// message which each participant will sign
    pub message: &'static [u8],
    /// signing_commitments are the set of commitments participants published
    /// in the first round of the protocol.
    pub signing_commitments: Vec<SigningCommitments>,
}

/// A participant's signature share, which the coordinator will use to
/// aggregate with all other signer's shares into the joint signature.
pub struct SignatureShare {
    /// index represents the participant index
    pub(crate) index: u32,
    /// signature is this participant's signature over the message
...
/// that are then used during signing. When performing signing using two rounds,
/// num_nonces would equal 1, to perform the first round. Batching entails
/// generating more than one nonce/commitment pair at a time.
/// Nonces should be stored in secret storage for later use, whereas
...
/// bound to a signing set, specific set of commitments, and a specific
/// message.
...
/// including that for this participant.
///
...
/// aggregates the shares into a signature to publish. This signature is
/// compatible with verification of a plain SpendAuth signature.
///
/// This operation is performed by a coordinator that can communicate with all
/// the signing participants before publishing the final signature. The
/// coordinator can be one of the participants or a semi-trusted third party
/// (who is trusted to not perform denial of service attacks, but does not
/// learn any secret information).
pub fn aggregate(
    signing_package: &SigningPackage,
