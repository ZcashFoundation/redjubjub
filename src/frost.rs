...
//! submodules. The signing protocol is coördinated by an untrusted aggregator.
//! This can be one of the share holders, but it is represented seperately
//! because it could be a third party. The entrypoint for the key generation
//! protocol is [`keygen::begin_keygen`], and the entrypoint for the signing
//! protocol is [`aggregator::begin_sign`].
//!
...
//! current state and produces the next state and message(s) to be sent to the
//! counterparties. It is the responsibility of the user of this module to handle
...
pub mod keygen;
/// States and messages for the participants in the signing protocol.
pub mod signer;
