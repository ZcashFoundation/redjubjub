...
/// A serializable & deserializable Scalar in RedJubJub.
#[derive(Copy, Clone, Debug, Default, Eq, PartialEq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[cfg_attr(feature = "serde", serde(try_from = "[u8; 32]"))]
#[cfg_attr(feature = "serde", serde(into = "[u8; 32]"))]
pub struct SerializableScalar(jubjub::Scalar);

impl From<SerializableScalar> for [u8; 32] {
    fn from(ss: SerializableScalar) -> [u8; 32] {
        ss.0.to_bytes()
    }
}

impl TryFrom<[u8; 32]> for SerializableScalar {
    type Error = &'static str;

    fn try_from(bytes: [u8; 32]) -> Result<Self, Self::Error> {
        // This checks that the encoding is canonical.
        let maybe_point = jubjub::Scalar::from_bytes(&bytes);
        if maybe_point.is_some().into() {
            let scalar: jubjub::Scalar = maybe_point.unwrap().into();
...
impl From<jubjub::Scalar> for SerializableScalar {
    fn from(source: jubjub::Scalar) -> SerializableScalar {
        SerializableScalar(source)
    }
}
/// A secret scalar value representing a single signer's secret key.
#[derive(Clone, Copy, Default, PartialEq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct Secret(SerializableScalar);
...
impl From<SerializableScalar> for Secret {
    fn from(source: SerializableScalar) -> Secret {
        Secret(source)
    }
}

/// A serializable & deserializable ExtendedPoint in RedJubJub.
#[derive(Copy, Clone, Debug, PartialEq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[cfg_attr(feature = "serde", serde(try_from = "[u8; 32]"))]
#[cfg_attr(feature = "serde", serde(into = "[u8; 32]"))]
pub struct SerializablePoint(jubjub::ExtendedPoint);

impl From<SerializablePoint> for [u8; 32] {
    fn from(sp: SerializablePoint) -> [u8; 32] {
        jubjub::AffinePoint::from(sp.0).to_bytes()
    }
}

impl TryFrom<[u8; 32]> for SerializablePoint {
    type Error = &'static str;
    ...
    fn try_from(bytes: [u8; 32]) -> Result<Self, Self::Error> {
        ...
        // This checks that the encoding is canonical...
        let maybe_point = jubjub::AffinePoint::from_bytes(bytes);
        if maybe_point.is_some().into() {
...
impl From<jubjub::ExtendedPoint> for SerializablePoint {
    fn from(source: jubjub::ExtendedPoint) -> SerializablePoint {
        SerializablePoint(source)
    }
}
...
#[derive(Copy, Clone, Debug, PartialEq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct Public(SerializablePoint);

impl From<SerializablePoint> for Public {
    fn from(source: SerializablePoint) -> Public {
        Public(source)
    }
}
...
#[derive(Clone, PartialEq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
struct Commitment(SerializablePoint);
...
pub fn keygen_with_dealer<R: RngCore + CryptoRng>(
    ...
    let secret = Secret(SerializableScalar(Scalar::from_bytes_wide(&bytes)));
    let group_public = VerificationKey::from(&secret.0 .0);
    ...
    for share in shares {
        let signer_public = Public(SerializablePoint(SpendAuth::basepoint() * share.value.0 .0));
...
fn verify_share(share: &Share) -> Result<(), &'static str> {
    let f_result = SpendAuth::basepoint() * share.value.0 .0;
    ...
    let (_, result) = share.commitment.0.iter().fold(
        ...
        |(x_to_the_i, sum_so_far), comm_i| (x_to_the_i * x, sum_so_far + comm_i.0 .0 * x_to_the_i),
    );
...
fn generate_shares<R: RngCore + CryptoRng>(
    ...
    commitment.0.push(Commitment(SerializablePoint(
    ...
    for c in &coefficients {
        commitment
            .0
            .push(Commitment(SerializablePoint(SpendAuth::basepoint() * c)));
    }
    ...
    for index in 1..numshares + 1 {
        ...
        shares.push(Share {
            ...
            value: Secret(SerializableScalar(value)),
...
pub fn sign(
    ...
    let signature: Scalar = participant_nonces.hiding
        ...
        + (lambda_i * share_package.share.value.0 .0 * challenge);
...
mod tests {
    ...
    fn reconstruct_secret(shares: Vec<Share>) -> Result<Scalar, &'static str> {
        ...
        for i in 0..numshares {
            secret += lagrange_coeffs[i] * shares[i].value.0 .0;
        }
    ...
    fn check_share_generation() {
        ...
        let _ = SpendAuth::basepoint() * secret.0 .0;
        ...
        assert_eq!(reconstruct_secret(shares).unwrap(), secret.0 .0)
    }
}
